...
// ⭐ NOUVEAU : Ajustement fréquences (±30%)
...
void setup() {
  ...
  Serial.print(F("Fréquence: ")); Serial.print(getFreqName(currentFreq));
  Serial.print(F(" (")); Serial.print(getAdjustedFreq(), 2); Serial.println(F(" Hz)"));
...
void handleButtons() {
  ...
  if (btnFreqPressed && (currentTime - lastDebounceFreq >= DEBOUNCE_DELAY)) {
    ...
    if (menuState == MENU_MAIN_DISPLAY) {
      ...
      Serial.print(F("Fréquence: ")); Serial.println(getFreqName(currentFreq));
...
void saveConfig() {
  ...
  EEPROM.write(EEPROM_ADDR_FREQ_ADJUST, freqAdjustIndex);       // 0/1/2
